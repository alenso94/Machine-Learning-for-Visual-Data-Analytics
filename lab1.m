...
%-----------------------------Write Your Own Code here that assigns all descriptors -------------
%-----------------------------Write Your Own Code here that assigns all descriptors -------------
index_train=zeros(1,length(TrainMat));%initialize index_train matrix with zeros of size 1 and length of TrainMat
index_test=zeros(1,length(TestMat));%initialize index_train matrix with zeros of size 1 and length of TrainMat

for i=1:length(TrainMat)%executes the looop all along the length of the train matrix
    discrptor_train = TrainMat(i,:);%assign the corresponding descriptor of TrainMat  in the loop as the discriptor_train
    d = EuclideanDistance(discrptor_train,C);%Find the euclidean distance between the discriptor_train and all the code words in the dictionary
    [minv,index_train(i)]=min(d);%Find the one with the minimum distance and assign the corresponding value to minv and corrresponding index to index_train(for the given index)
end
for i=1:length(TestMat)%executes the looop all along the length of the test matrix
    discrptor_test = TestMat(i,:);%assign the corresponding descriptor of TestMat  in the loop as the discriptor_test
    d = EuclideanDistance(discrptor_test,C);%Find the euclidean distance between the discriptor_test and all the code words in the dictionary
    [minv,index_test(i)]=min(d);%Find the one with the minimum distance and assign the corresponding value to minv and corrresponding index to index_t(for the given index)
end
...
wordid =19; %12;% set a random value and we intead to find 20 patches from both training and test
...
% %{%
    ...
    for ii = 1:nimages
          ...
          coWords = zeros(1,vocbsize);%create a matrix of length same as
          %number of code words
          s = size(d,1);%find %assign the number of rows of d to size
          for i = 1:s%execute the loop for d number of times
              [min_val,min_ind] = min(d(i,:));% find the minimum distance
              %among all the code words and assign the minimum value and its
              %corresponding index to min_val and min_ind
              coWords(1,min_ind) = coWords(1,min_ind)+1;%increment the value
              %at the corresponding index where to which the distance is
              %minimum
          end
          histogram = do_normalize(coWords);%Normalise the histograms by their L1 norm
          BoW(ii,:) = histogram;%The normalized histograms are set to BoW for corresponding image 
          
          if isshow == 1
            close all; figure;
            subplot(1,2,1),imshow(imread(strcat('image/',image_names{ii})));
    
            subplot(1,2,2),bar(BoW(ii,:)),xlim([0 1500]);
          end 
    end 
...
idright = right_v(10);% set number you are about to visualize
...
%==========================================================================
%% 4.5.1 K-NN training using histogram intersection
%--------------------------code--------------------------------------------
%{%
clear;
load('data/global/bow');
...
% % 4.5.4 Show some correct and incorrect cases 
% %---------------------------------code-------------------------------------
% {% 
load('data/global/image_names');
load('data/global/bow');
...
n_w = length(wrong_v);
nv = min(n_w,20);
nrows = floor((nv*2*1.1)/5);
close all;
figure 
for k = 1:nv
    subplot(nrows,5,2*k-1),imshow(imread(strcat('image/',image_names{right_v(k)+300}))),title(strcat('PL-',class_names{predict_label(right_v(k))})),axis tight,axis off;
    subplot(nrows,5,2*k),imshow(imread(strcat('image/',image_names{wrong_v(k)+300}))),title(strcat('PL-',class_names{predict_label(wrong_v(k))})),axis tight,axis off;
end 
%}
%-------------------------------end of code--------------------------------