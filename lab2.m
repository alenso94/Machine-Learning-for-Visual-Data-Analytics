...
clear
%addpath software;
addpath(genpath('matlab_files'));%path is chosen as matlab_files as all mfiles are present in it

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Loading of the images: You need to replace the directory 

Imagestrain = loadImagesInDirectory ( 'training-set/23x28/');%Training images are loaded to Imagestrain
[Imagestest, Identity] = loadTestImagesInDirectory ( 'testing-set/23x28/');%Test images are loaded to Imagestest and their corresponding identities are loaded ro Identity
...
for j = 1:20%to loop through first 20 eigen faces
    subplot(4,5,j)%divide the figure into 4 rows and 5 columns to accomodate 20 eigen faces
    Eigenface=reshape(V(:,j),28,23);%the eigen vectors are selcted for reach eigen faces and it is reshaped into size 28*23
    imshow(Eigenface, []);    %eigen face is displayed for each eigen vecctors after scaling the image to full scale. 
    title(['EigenFace ' num2str(j)]); %title is displayed for each eigen vector by converting the number to string. 
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Threshold =20;  %20 eigen faces
Distances=zeros(TestSizes(1),TrainSizes(1));%distance matrix to store distance between each test image and train image

for i=1:TestSizes(1),
    for j=1: TrainSizes(1),
        Sum=0;
        for k=1: Threshold,
   Sum=Sum+((Locationstrain(j,k)-Locationstest(i,k)).^2);%find distance between 2 images
        end,
     Distances(i,j)=Sum;%distance between each test image and train image are stored in distance matrix.
    end,
end,
...
[Values(i,:), Indices(i,:)] = sort(Distances(i,:));%distance is sorted per row to find the nearest neighbour
end,

recognised_person=zeros(1,20);
...
while (i<70),
    id=Identity(1,i);   
    distmin=Values(id,1);%minimum distance
        indicemin=Indices(id,1);%index of minimum distance
    while (i<70)&&(Identity(1,i)==id), 
        if (Values(i,1)<distmin),
            distmin=Values(i,1);%minimum distance
        indicemin=Indices(i,1);%index of minimum distance
        end,
        i=i+1;
...
for  i=1:5,
   recognitionrate(1,i)=recognitionrate(1,i)/number_per_number(1,i);%find recognition rate for 20 eigen faces
end,
rec_rate_20=mean(recognitionrate(1,:));%find average recognition rate for 20 eigen faces
    

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
%effect of threshold (i.e. number of eigenfaces):  
averageRR=zeros(1,40);
for t=1:40,
  Threshold =t;  
Distances=zeros(TestSizes(1),TrainSizes(1));
...
averageRR = zeros(1,20);%initialising average recognition rate for 20 eigen faces
Threshold =20;%20 eigen faces
Distances=zeros(TestSizes(1),TrainSizes(1));%initialize distance matrix
...
for i=1:70,
number_of_test_images(1,Identity(1,i))= number_of_test_images(1,Identity(1,i))+1;%YY I modified here
[Values(i,:), Indices(i,:)] = sort(Distances(i,:));%distance is sorted per row to find the nearest neighbours
end,
...
for i=1:70;
    ...
    for j=1:K,
        for k=j:K,
            if (person(i,k)==person(i,j))
                number_of_occurance(i,j)=number_of_occurance(i,j)+1;%increment the number of occurances if that person is found
            end,
        end,
 ...
 if (Identity(1,i)==recognised_person(1,i))
     recognitionrate=recognitionrate+1;%increment if the person has minimum distances
 end,

averageRR(1,K)=recognitionrate/70;%divide the recognition rate per the number of test images 
end,
end,

figure;
plot(averageRR(1,:));%plot the averageRR
